...
class MasterViewController: UITableViewController, InsertViewControllerDelegate {
    ...
    var cityNames = ["Dallas", "Warsaw", "Rome"]
    var cities = [CityCell](){didSet{tableView.reloadData()}}

    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Dominik Bazan"
    ...
    func fetchCityExtendedInfoByName(cityName: String) {
        let url = URL(string: ("https://www.metaweather.com/api/location/search/?query=" + cityName).addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!)!
    ...
    func fetchCityExtendedInfoById(cityName: String, id: String) {
        URLSession(configuration: .default).dataTask(with: URL(string: "https://www.metaweather.com/api/location/" + id)!, completionHandler: { (data, response, error) in
            guard let newData = data else { return }
            let decoder = JSONDecoder()
            do {
                ...
                let city = CityCell(id: id, name: cityName, temperature: temperature, imageView: image!)
